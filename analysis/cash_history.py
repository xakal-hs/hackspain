...
def load_sources(connection: duckdb.DuckDBPyConnection, data_dir: Path | None = None) -> None:
    """Registra las vistas mapeadas sobre los CSV. El país ya llega en ISO-2."""
    ...
    attach(connection, data_dir or (root / "data"))
...
def build_cash_sections(connection: duckdb.DuckDBPyConnection, ui: Any) -> str:
    ...
    <section class="section" id="caja-metodo">
      <div class="section-head"><span>06</span><div><h2>Caja reconstruida: método y validación</h2><p>El histórico mensual de caja no existe en el dataset: se reconstruye hacia atrás desde la foto de saldos y se valida contra ella.</p></div></div>
      <div class="kpis">
    ...
    <section class="section" id="caja-evolucion">
      <div class="section-head"><span>07</span><div><h2>Cómo ha evolucionado la caja</h2><p>Nivel, dispersión y trayectoria de las {fmt(n_companies_panel, 0)} series reconstruidas.</p></div></div>
    ...
    <section class="section" id="caja-temporalidad">
      <div class="section-head"><span>08</span><div><h2>Temporalidad: el calendario fiscal manda</h2><p>Estacionalidad normalizada dentro de cada empresa, para que el tamaño y la incorporación progresiva no se confundan con ciclo.</p></div></div>
    ...
    <section class="section" id="caja-grupos">
      <div class="section-head"><span>09</span><div><h2>Grupos: la caja no está donde hace falta</h2><p>La unidad de negocio es la empresa, pero la tesorería se gestiona en el grupo.</p></div></div>
      <div class="kpis compact-kpis">
    ...
    <section class="section" id="caja-problemas">
      <div class="section-head"><span>10</span><div><h2>Problemas de caja: diagnóstico y anticipación</h2><p>Estado de cada empresa en su último mes observado, separando el bache del deterioro y midiendo con cuánto margen se veía venir.</p></div></div>
      <div class="grid two">
...
def _build_metrics(connection: duckdb.DuckDBPyConnection) -> None:
    ...
    connection.execute(
        ...
        SELECT l.company_id,
               -- `country` llega ya canonizado a ISO-2 por la capa de mapeo; el texto libre
               -- original queda en `country_raw` para los recuentos de calidad.
               c.group_id, c.country, c.currency, c.erp,
