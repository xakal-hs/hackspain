...
- The public landing is self-contained in `frontend/app/pages/index.vue`: fixed dark marketing palette, CSS aurora with pause and reduced-motion support, illustrative demo charts, and CTAs to `/login`. Its header replaces the old landing rail described in `frontend/DESIGN_SYSTEM.md`; dashboard and login themes remain unchanged.
