...
- Frame treasury recommendations with loss aversion: a side panel with two scenarios (cost of not acting vs following the advice), as money already being lost. Financing-request email drafts stay generic — no real company names, IDs, or amounts.
...
- The reproducible exploratory report consists of `analysis/generate_report.py` and the generated self-contained artifact `analysis/report.html`. Historical cash is reconstructed in `analysis/cash_history.py` (formula, sentinel handling, cent rounding, drift flags) and explored in `analysis/app.py`. Persist with `scripts/build_cash_db.py`; do not commit `analysis/cash.duckdb`.
...
- Score-model experiments, the research API, and the research demo SPA live under `research/` (visual language in `research/app/DESIGN.md`). The jury-facing product UI is the Nuxt app in `frontend/` (`pnpm dev` → http://localhost:3000); it falls back to five fixture companies when Supabase is not configured. The public landing is self-contained in `frontend/app/pages/index.vue` (fixed dark marketing palette, CSS aurora, CTAs to `/login`). Equipo Embat is two screens on live treasury: Caja (`/dashboard/embat`, alerts vs opportunities from `featured_companies` + `panel_monthly`) and a financing CRM (`?section=crm`; schema `supabase/embat_crm_schema.sql`, local fallback `frontend/.data/` until applied). The frontend public-schema Supabase project is separate from the private `xray` project used by `scripts/push_supabase.py`.
- The score explanation UI groups raw driver contributions into the five `backend/preprocessing.py` pillars via `frontend/app/utils/scorePillars.ts`. These are summed points, not backend `p_*` subscores or month-over-month deltas; rules/unknown features remain separate. The API returns `feature` for stable mapping, with label fallback for legacy/demo rows. `node --test frontend/tests/*.test.mjs` on Node 22.22 validates catalogue parity, precision, missing data and sector alignment.
- **Deployment is Nuxt-only.** `frontend/` is the sole deployed service: Nitro serves every product route (`/api/companies`, `/api/companies/:id`, `…/explain`, `…/decision`, `/api/model`, `/api/vetos`, `/api/chat`) by reading the published contract in Supabase, and the agent's tools call those same routes internally. `backend/` is the offline job that computes, not an API to run; `XRAY_API_BASE` no longer exists. Heavy aggregation lives in the `company_portfolio_latest` view and the portfolio read is cached 5 min. See `frontend/DEPLOYMENT.md`.
- The published score is the `xray-v7` lineage (`research/src/xray.py` → `company_health_monthly`). `backend/predict.py` is a **different** score: same rows and band thresholds, but the band disagrees on 11,4 % of them. Do not mix them. The lender decision (`backend/decision.py`: 8 vetoes, acción, importe recortado por cobertura) is published by `scripts/export_decision_publication.py`, which feeds `decide()` the already-published score so the action always matches the number shown.
