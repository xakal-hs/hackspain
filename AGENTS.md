...
- Score-model experiments, the research API, and the research demo SPA live under `research/`. The jury-facing product UI is the Nuxt app in `frontend/` (`pnpm dev` → http://localhost:3000); it runs on five fixture companies when Supabase is not configured.
