...
- Score-model experiments, the research API, and the research demo SPA live under `research/` (visual language in `research/app/DESIGN.md`). The jury-facing product UI is the Nuxt app in `frontend/` (`pnpm dev` → http://localhost:3000). Equipo Embat is two screens on live treasury: Caja (`/dashboard/embat`, alerts vs opportunities from `featured_companies` + `panel_monthly`) and a financing CRM (`?section=crm`; schema `supabase/embat_crm_schema.sql`, local fallback `frontend/.data/` until applied). The frontend public-schema Supabase project is separate from the private `xray` project used by `scripts/push_supabase.py`.
