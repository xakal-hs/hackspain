...
| Fallo | Evidencia | Acción | Salida |
|---|---|---|---|
| Saldos absurdo | `PRODUCT_00001` ≈ 1e11; umbral \|balance\| > 1e8 | No recortar; marcar | `balance_quality` (`ok`/`suspect`), `dq_balance_suspect` |
...
| `exchange_rate` ≤ 0 | 77 | Null | `exchange_rate`, `exchange_rate_raw` |
| `counterparty_id` ancho variable | Txn 5 dígitos; facturas 59 856 con 6 | Pad a 6 para join | `counterparty_id`, `counterparty_id_raw` |
...
- Conversión FX a moneda de empresa.
- Reconstrucción histórica de saldos.
