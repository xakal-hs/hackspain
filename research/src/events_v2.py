...
E2  Incumplimiento estricto de obligación recurrente (nómina, cuota, IVA en 2 trimestres, AP>90d)
...
def build(p: pd.DataFrame) -> pd.DataFrame:
    d = prepare(p)
    d = e1(d); d = e1(d, liquidity=True); d = e2(d); d = e3(d); d = e4(d); d = e5_e6_e7(d)
    return d
